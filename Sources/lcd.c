...
void connectGpio(void){
	//Configure CS [PTD0] pin as GPIO ALT1
	PORTD_PCR0 |= PORT_PCR_MUX(0x01); //MUX[bit 8-10] = 001b
	GPIOD_PDDR |= GPIO_PDDR_PDD(1<<0); // PTD0 as an output
	GPIOD_PSOR |= (uint32_t)(1<<0);//
	//Configure MOSI [PTD2] pin as GPIO as ALT1
	PORTD_PCR2 |= PORT_PCR_MUX(0x01);
	GPIOD_PDDR |= GPIO_PDDR_PDD(1<<2);
	GPIOD_PCOR |= (uint32_t)(1<<2);// init value low
	//Configure CLK [PTD1] pin as GPIO as ALT1
	PORTD_PCR1 |= PORT_PCR_MUX(0x01);
	GPIOD_PDDR |= GPIO_PDDR_PDD(1<<1);
	GPIOD_PCOR |= (uint32_t)(1<<1);
	//configure RESET - PTC4
	PORTC_PCR4 |= PORT_PCR_MUX(0x01);
	GPIOC_PDDR |= GPIO_PDDR_PDD(1<<4);
	GPIOC_PSOR |= (uint32_t)(1<<4);
	//Green LED
	PORTE_PCR26 |= PORT_PCR_MUX(0x01); //PTE26 Green LED alt1
	GPIOE_PDDR |= GPIO_PDDR_PDD(1<<26);//set as an output
	GPIOE_PCOR |= (uint32_t)(1<<26);//Clear to light LED
}


void connectSpi(void){
	SM1_ConnectPin(SM1_SS_PIN);
	SM1_ConnectPin(SM1_CLK_PIN);
	SM1_ConnectPin(SM1_OUT_PIN);
}
...
void CS_ClrVal(void){
	GPIOD_PDOR &= ~(0x01); //clear CS - PTD0
}
void CS_SetVal(void){
	GPIOD_PDOR |= (uint32_t)1; //set CS - PTD0
}
void MOSI_ClrVal(void){
	GPIOD_PDOR &= ~((uint32_t)(1<<2));// clear MOSI - PTD2
}
void MOSI_SetVal(void){
	GPIOD_PDOR |= (uint32_t)(1<<2);// set MOSI - PTD2
}
void SCK_ClrVal(void){
	GPIOD_PDOR &= ~((uint32_t)(1<<1));// set SCK - PTD1
}
void SCK_SetVal(void){
	GPIOD_PDOR |= 1<<1;// set SCK - PTD1
}
void RESET_ClrVal(void){
	GPIOC_PDOR &= ~((uint32_t)(1<<4));//clear PTC4
}
void RESET_SetVal(void){
	GPIOC_PDOR |= (uint32_t)(1<<4);//set PTC4
}
...
void lcd_send(uint16_t bytes){

	if(HWSPI==1){
		SM1_SendChar((SM1_TComData)bytes);
		/*// change MOSI, SCK and CS back to SPI0 ALT2
		SM1_Enable();
		connectSpi();
		SM1_SendChar((uint8_t)bytes);
		SM1_Disable();
		connectGpio();
		CS_ClrVal();//PTD0 CS clear
		if(bytes & 256)
			MOSI_SetVal();
		else
			MOSI_ClrVal();
		//toggle the clock to transmit 1 bit
		SCK_SetVal();//PTD1 ALT1, SPI0_SCK ALT2
		SCK_ClrVal();
		CS_SetVal();*/
	}else{
		uint16_t i;
